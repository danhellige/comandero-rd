<?php
/**
 * Procesador de órdenes RD - Servicio independiente
 * Se ejecuta cada 10 segundos via servicio Windows
 */

require_once __DIR__ . '/connection.php';

// Función para procesar tickets y guardar en ordenes_rd
function procesarYGuardarTickets($connect) {
    ...
    if ($procesados > 0) {
        logAudit("SERVICIO: Procesados $procesados tickets nuevos");
    }
...
function procesarTicketIndividual($connect, $ticket) {
    ...
    while ($linea = mysqli_fetch_assoc($result_lineas)) {
        ...
        // Detectar indicadores de tipo de servicio
...
// Actualizar timestamp de ejecución y último ticket
$ultimo_ticket = mysqli_query($connect, "SELECT MAX(ticketid) as ultimo FROM ordenes_rd WHERE DATE(fecha_orden) = CURDATE()");
$row = mysqli_fetch_assoc($ultimo_ticket);
$ticket = $row['ultimo'] ?? 0;

mysqli_query($connect, "UPDATE rd_status SET ultima_ejecucion = NOW(), ultimo_ticket = $ticket WHERE id = 1");
